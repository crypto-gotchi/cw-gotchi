...
use crate::state::{Approval, Cw721Contract, TokenInfo, WearableOwner};
    ...
    Q: CustomMsg,
{
    pub fn mint(
        ...
        // create the token
        let token = TokenInfo {
            owner: WearableOwner::Address(deps.api.addr_validate(&owner)?),
            approvals: vec![],
            token_uri,
            extension,
        };
    ...
    Q: CustomMsg,
{
    pub fn _transfer_nft(
        ...
        let owner = WearableOwner::from_str(recipient)?;
        match &owner {
            WearableOwner::Address(addr) => {
                deps.api.addr_canonicalize(addr.as_str())?;
            }
            _ => {}
        }
    ...
    pub fn check_can_approve(
        ...
        // owner can approve
        let owner_address = token.owner.address()?;

        if owner_address == info.sender {
            return Ok(());
        }
        // operator can approve
        let op = self
            .operators
            .may_load(deps.storage, (&owner_address, &info.sender))?;
        match op {
            Some(ex) => {
